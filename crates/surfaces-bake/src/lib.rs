...
//! decimal coordinates do not overflow. A finite residual whose ceil does
//! not fit in `i32` is [`BakeError::BoundOverflow`]. Host `f64` lerp is not
//! the bound oracle. For exact `u16` coordinates that includes the
//! runtime-rounded X-then-Y path. It is not a typical error, and not a
//! device, vendor, sensor, calibration, or accuracy claim.
//!
