...
//! exact `u16` coordinates that includes the runtime-rounded X-then-Y path
//! and the unrounded rational bilinear of the `i32` grid. A nonzero computed
//! magnitude is stepped one ULP before `ceil` so a residual that rounded
//! downward onto an integer cannot understate; exact-zero tables stay 0. It
//! is not a typical error, and not a device, vendor, sensor, calibration, or
//! accuracy claim.
//!
